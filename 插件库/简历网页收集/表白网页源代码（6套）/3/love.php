...
if(isset($_GET['add']) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	if(preg_match('/^[\w\-]{3,30}$/',$textHref) == 0){
		...
		if(file_exists(dirname(__FILE__) . "/2016/$textHref.html")){
			$return = array ('status'=>0,'msg'=>'←这个链接已存在');
			$path_status = "失败：存在";
		}else{
			$path = dirname(__FILE__) . "/2016/$textHref.html"; // 保存为
			$handle = fopen($path,"w");
			if($handle){
				$return = array ('status'=>1,'url'=>'http://'.$_SERVER['HTTP_HOST'].'/yanshi/biaobai/3/2016/'.$textHref.'.html');
				$path_status = "成功：生成";
			}else{
